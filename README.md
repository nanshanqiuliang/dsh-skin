...
| 鼠标涟漪（图片皮肤） | 鼠标涟漪（视频皮肤） |
|---|---|
| ![涟漪图片](screenshots/09-鼠标涟漪-图片皮肤.png) | ![涟漪视频](screenshots/10-鼠标涟漪-视频皮肤.png) |
...
| 星空闪烁 / 流星 | 在背景上加动态粒子效果 |
| **鼠标涟漪 / 涟漪强度** | 开关「鼠标划过时背景像水面被扰动、泛起涟漪」；强度滑杆 10%~300%。停下鼠标后涟漪会自动淡出并交还背景（省电） |
...
开启后鼠标在页面上移动，背景会像水面被划过一样荡开涟漪 —— 图片皮肤和视频皮肤都支持。

- **实现**：波动用二维水面波动方程（双缓冲高度场，固定时间步长，帧率低时不会变慢动作）；
  渲染走 **WebGL**：波动场作为小纹理上传，着色器里按波高梯度做 UV 折射 + 曲率高光，
  图片/视频直接当纹理采样，**不做 getImageData 回读** —— 所以输出是**屏幕全分辨率**（不再降低背景画质），
  也不会拖慢视频解码（实测涟漪开启时视频仍满帧播放）。拿不到 WebGL 时自动退回 2D canvas 路径。
- **性能**：只在你移动鼠标期间渲染，停手 1~3.5 秒后自动收起并交还背景；标签页隐藏时暂停。
- **开源来源与许可**：见 [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md)（波动算法参考 MIT 许可的 victorqribeiro/rippleEffect 与 Hugo Elias 的公开算法说明；WebGL 渲染思路参考 jquery.ripples）。
