...
- **实现**：二维水面波动方程（双缓冲高度场迭代）+ 用波高梯度对背景做折射采样 + 曲率高光；画布分辨率自适应（约 15 万像素上限），靠 CSS 放大，配合原有虚化足够清晰。
- **性能**：停手约 1~4 秒后自动停止渲染并交还背景（不会一直烧 CPU）；标签页隐藏时也会暂停。
- **开源来源与许可**：见 [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md)（波动算法参考 MIT 许可的 victorqribeiro/rippleEffect 与 Hugo Elias 的公开算法说明）。
