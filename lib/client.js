...
`
    ...
    function ensureLayer() {
      ...
      ripple.ctx = rippleCanvas.getContext('2d', { willReadFrequently: true })
      return layer
    }
    ...
    /* ── 鼠标涟漪（水面扰动）─────────────────────────────────────────────
       ...
       实现参考 MIT 许可的开源项目 victorqribeiro/rippleEffect（jquery.ripples 同源思路），
       并扩展为：用波高梯度对背景（图片或视频）做折射采样 + 曲率高光，
       所以图片皮肤和视频皮肤都能被鼠标扰动。
       ──────────────────────────────────────────────────────────────── */
    const ripple = {
      canvas: null, ctx: null, srcCanvas: null, srcCtx: null,
      w: 0, h: 0, wave: null, prev: null, out: null, imageData: null,
      running: false, raf: 0, energy: 0, idle: 0,
      damping: 0.976, strength: 1, lastDropAt: 0, source: null, kind: 'image',
      lastX: -999, lastY: -999, enabled: false, active: false, imageCache: null,
    }
    ...
    function rippleResize() {
      ...
      const scale = Math.min(0.5, Math.sqrt(maxPixels / (cssW * cssH)))
      const w = Math.max(160, Math.round(cssW * scale))
      const h = Math.max(120, Math.round(cssH * scale))
      if (w === ripple.w && h === ripple.h) return true
      ripple.w = w
      ripple.h = h
      canvas.width = w
      canvas.height = h
      ripple.wave = new Float32Array(w * h)
      ripple.prev = new Float32Array(w * h)
    ...
    function rippleDrop(x, y, power) {
      ...
      const cx = Math.round(clamp(x, 1, w - 2))
      const cy = Math.round(clamp(y, 1, h - 2))
      const p = cy * w + cx
      ripple.wave[p] = Math.min(700, ripple.wave[p] + 380 * power)
    ...
    /** 把背景素材按 cover 画进小画布，返回像素数据。 */
    function rippleSource() {
      const w = ripple.w, h = ripple.h
      const ctx = ripple.srcCtx
      let source = ripple.source
      let sw = 0, sh = 0
      if (source === null) return null
      sw = source.videoWidth || source.naturalWidth || source.width || 0
      sh = source.videoHeight || source.naturalHeight || source.height || 0
      if (sw === 0 || sh === 0) return null
      if (ripple.kind === 'image' && ripple.imageCache !== null) return ripple.imageCache
    ...
    function rippleDraw() {
      const srcData = rippleSource()
      if (srcData === null) return
      const w = ripple.w, h = ripple.h
      const wave = ripple.wave, out = ripple.out
      const disp = clamp(ripple.strength, 0.1, 3) * 0.85
      for (let y = 1; y < h - 1; y += 1) {
        let p = y * w + 1
        for (let x = 1; x < w - 1; x += 1, p += 1) {
          const hl = wave[p - 1], hr = wave[p + 1], hu = wave[p - w], hd = wave[p + w]
    ...
    function rippleTick() {
      ...
      const energy = rippleStep()
      const idleMs = Date.now() - ripple.lastDropAt
      if (energy > 2.5 || idleMs < 1200) {
        rippleDraw()
        ripple.idle = 0
      } else {
        ripple.idle += 1
        if (ripple.idle > 8 || idleMs > 4000) { rippleStop(); return }
      }
    ...
    function rippleStop() {
      ...
      if (layer !== null) {
        if (current !== null && current.kind === 'video') layer.video.style.display = 'block'
        else if (current !== null) layer.media.style.backgroundImage = `url("${current.url}")`
      }
    }

    function rippleMouse(event) {
      if (!ripple.enabled || ripple.canvas === null) return
      if (ripple.w === 0 && !rippleResize()) return
      const rect = ripple.canvas.getBoundingClientRect()
      if (rect.width === 0 || rect.height === 0) return
      const x = (event.clientX - rect.left) / rect.width * ripple.w
      const y = (event.clientY - rect.top) / rect.height * ripple.h
      const dist = Math.hypot(x - ripple.lastX, y - ripple.lastY)
      if (dist < 2.5) return
      const power = clamp(dist / 22, 0.3, 1.5)
      ripple.lastX = x
      ripple.lastY = y
    ...
    /** 由 applySkin 调用：按当前皮肤开关涟漪，并接管/交还背景显示。 */
    function rippleApply(skin, opts) {
      ...
      if (ripple.kind === 'video') {
        ...
        image.onload = () => { ripple.imageCache = null; ripple.source = image }
        image.src = skin.url
        ripple.source = null
      }
    }
